...
def _load_metadata_safe(excel_retriever, file_id: str) -> Optional[Dict[str, Any]]:
    """
    Safely load file metadata, handling both sync and async retrievers.
    
    Args:
        excel_retriever: ExcelRetriever instance (sync or async)
        file_id: File ID to load metadata for
        
    Returns:
        Metadata dictionary or None
    """
    if not excel_retriever or not file_id:
        return None
    
    try:
        # Try sync wrapper first (MongoDBExcelRetriever)
        if hasattr(excel_retriever, 'load_file_metadata_sync'):
            return excel_retriever.load_file_metadata_sync(file_id)
        
        # Check if it has load_file_metadata method
        if hasattr(excel_retriever, 'load_file_metadata'):
            # Check if it's async
            if inspect.iscoroutinefunction(excel_retriever.load_file_metadata):
                ...
                try:
                    ...
                    if loop.is_running():
                        ...
                        return run_async_in_thread(excel_retriever.load_file_metadata(file_id))
                    else:
                        # No running loop, can use asyncio.run
                        return asyncio.run(excel_retriever.load_file_metadata(file_id))
                except RuntimeError:
                    # No event loop, create one
                    return asyncio.run(excel_retriever.load_file_metadata(file_id))
            else:
                # Synchronous version
                return excel_retriever.load_file_metadata(file_id)
...
def create_excel_retriever_tool(excel_retriever, semantic_retriever, user_id: Optional[str] = None) -> Tool:
    ...
    def retrieve_data(query: str) -> str:
        ...
        try:
            ...
            # Step 2: Use semantic search to find relevant columns (with user_id filtering)
            columns = []
            if semantic_retriever:
                # Check if semantic_retriever has async retrieve_columns
                import inspect
                if inspect.iscoroutinefunction(semantic_retriever.retrieve_columns):
                    # Run async function synchronously
                    import asyncio
                    import concurrent.futures
                    ...
                    try:
                        ...
                        if loop.is_running():
                            # If loop is running, run in a separate thread with new event loop
                            columns = run_async_in_thread(
                                semantic_retriever.retrieve_columns(query, n_results=10, user_id=user_id)
                            )
                        else:
                            # No running loop, can use asyncio.run
                            columns = asyncio.run(semantic_retriever.retrieve_columns(query, n_results=10, user_id=user_id))
                    except RuntimeError:
                        # No event loop, create one
                        columns = asyncio.run(semantic_retriever.retrieve_columns(query, n_results=10, user_id=user_id))
                else:
                    # Synchronous version
                    columns = semantic_retriever.retrieve_columns(query, n_results=10, user_id=user_id)
            ...
            if file_id and not columns:
                # Load metadata to get column names (handle both sync and async)
                metadata = _load_metadata_safe(excel_retriever, file_id)
            ...
            if not file_id:
                return json.dumps({
                    "success": False,
                    "error": f"Could not find file matching query: '{query}'. Available files: {[f.get('original_filename') for f in (excel_retriever.list_all_files_sync() if hasattr(excel_retriever, 'list_all_files_sync') else excel_retriever.list_all_files())]}"
                })
            ...
            if columns:
                ...
                # Ensure we only get strings, not dicts or None values
                matching_columns = [
                    col.get("column_name") 
                    for col in columns[:10] 
                    if col.get("file_id") == file_id and col.get("column_name")
                ]
            ...
            # Ensure columns_to_retrieve is clean (no dicts, no None values)
            clean_columns = None
            if columns_to_retrieve:
                ...
                clean_columns = clean_columns if clean_columns else None
            ...
            if result.get("success"):
                ...
                # Enhance schema_note to be more prominent with all columns
                available_columns = result.get("columns", [])
                if available_columns:
                    # Ensure all columns are strings (not dicts)
                    clean_column_names = []
                    for col in available_columns:
                        if isinstance(col, str):
                            ...
                            col_name = col.get("column_name") or col.get("name")
                            if col_name and isinstance(col_name, str):
                                clean_column_names.append(col_name)
                        else:
                            try:
                    ...
                    if clean_column_names:
                        result["schema_note"] = f"⚠️ IMPORTANT: Available columns in this file: {', '.join(clean_column_names)}. Use these EXACT column names in all subsequent queries. Do NOT assume column names - always use the names listed here."
                        result["all_columns"] = clean_column_names  # Add explicit field for agent to check
...
def create_trend_analyzer_tool(trend_analyzer, excel_retriever=None, semantic_retriever=None, user_id: Optional[str] = None) -> Tool:
...
def create_comparative_analyzer_tool(comparative_analyzer, excel_retriever=None, semantic_retriever=None, user_id: Optional[str] = None) -> Tool:
...
def create_kpi_calculator_tool(kpi_calculator, excel_retriever=None, semantic_retriever=None, user_id: Optional[str] = None) -> Tool:
...
def create_graph_generator_tool(graph_generator, excel_retriever=None, semantic_retriever=None, user_id: Optional[str] = None) -> Tool:
